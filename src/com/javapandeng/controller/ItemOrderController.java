...
public class ItemOrderController  extends BaseController {
    ...
    public String exAdd(@RequestBody List<CarDto> list, HttpServletRequest request){
        ...
        if(attribute==null){
            js.put(Consts.RES,0);//没有登录  前端用的是0
        ...
        BigDecimal to = new BigDecimal(0);//总价
        for(CarDto c:list){
            ids.add(c.getId());
        ...
        order.setIsDelete(0);
        order.setTotal(to.setScale(2,BigDecimal.ROUND_HALF_UP).toString());//截断  保留两位
